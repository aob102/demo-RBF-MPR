...
&{DICT1}          un=${1}    deux=${2}    ${3}=trois    TEST=XXXXX
...
TC Acces Variables And Log Variables
    ...
    Log Report And Console    XXXX${SPACE * 5}XXXX
...
TC Run Keyword If
    ...
    BuiltIn.Run Keyword If    ${True}    Log Report And Console    OK
    BuiltIn.Run Keyword If    not(${False})    Log Report And Console    OK
    BuiltIn.Run Keyword Unless    ${False}    Log Report And Console    OK
    BuiltIn.Run Keyword If    '${myString}' == 'TEST'    Log Report And Console    OK
    ...
    BuiltIn.Run Keyword If    '''${myString2}''' == '''c'est mon test'''    Log Report And Console    OK
    # la condition se fait si une sous chaine de caractère existe dans une autre chaine de caractère
    BuiltIn.Run Keyword If    """c'est""" in """${myString2}"""    Log Report And Console    OK
    # la condition se fait si la chaine de caractère existe dans une liste
    BuiltIn.Run Keyword If    'un' in @{MANY_LIST}    Log Report And Console    OK
    # combiner les conditions
    BuiltIn.Run Keyword If    ('un' in @{MANY_LIST} and ('${myString}' == 'X' or '${myString}' == 'TEST'))    Log Report And Console    OK
...
TC Run Keyword If And Set Variable
    ...
    ${myString1}    BuiltIn.Run Keyword If    ${myNumber} > 0    BuiltIn.Set Variable    NEW_VALUE
    ...
    ${myString1}    BuiltIn.Run Keyword If    ${FALSE}    BuiltIn.Set Variable    NEW_VALUE
    ...    ELSE    BuiltIn.Set Variable    ${myString1}
    # ou affecter une nouvelle valeur
    ${myString1}    BuiltIn.Run Keyword If    ${FALSE}    BuiltIn.Set Variable    NEW_VALUE
    ...    ELSE    BuiltIn.Set Variable    SECOND_VALUE
...
TC Should Contain FAIL3
    ...
    BuiltIn.Should Be Equal    ${myNumber}    ${myString}    msg=Les deux valeurs sont différentes : ${myNumber} <> ${myString}    values=False
...
TC Run Keyword And Return Status
    ...
    # gestion de l'erreur
    BuiltIn.Run Keyword If    ${status} == ${False}    Log Report And Console    ACTION A REALISER EN CAS D'ERREUR
...
TC Run Keyword And Ignore Error PASS
    ...
    BuiltIn.Run Keyword If    '${status}' == 'FAIL'    Log Report And Console    ACTION A REALISER EN CAS D'ERREUR
...
TC Run Keyword And Ignore Error FAIL
    ...
    # ...
    BuiltIn.Run Keyword If    '${status1}' == 'FAIL' and '${status2}' == 'FAIL'    BuiltIn.Fail    TEST EN ERREUR\nstatut1=${status1}, message1=${value1}\nstatut2=${status2}, message2=${value2}
...
TC Exit For Loop
    ...
    FOR    ${INDEX}    IN RANGE    10
        Log Report And Console    ${INDEX}
        BuiltIn.Exit For Loop If    ${INDEX} == 3
    END
...
TC Keyword with parameters In - Out
    ...
    ${resultat}    SOMME    ${4}    ${5}
    Log Report And Console    ${resultat}
...
TC Check Common Tags
    ...
    [Tags]    TA    TB    TC
    BuiltIn.Run Keyword If    'TA' in ${TEST TAGS}    Log Report And Console    'TA' Exist in ${TEST TAGS}
...
testTeardown
    ...
    BuiltIn.Run Keyword If    '${TEST STATUS}' == 'PASS'    BuiltIn.Set Test Message    Test OK\nOn pourait mettre la synthèse des étapes/vérificarions effectuées
